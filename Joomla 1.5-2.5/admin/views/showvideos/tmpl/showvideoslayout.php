...
<form action="index.php?option=com_contushdvideoshare&layout=adminvideos<?php echo $userUrl; ?>"
	...
	<fieldset id="filter-bar">
		<div class="filter-search fltlft" style="float: left;">
			...
			<input type="text" title="Search in module title." value="<?php if (isset($videolist1['lists']['search']))
			...
			<button onclick="document.getElementById('search').value='';this.form.submit();" type="button" style="padding: 1px 6px;"><?php echo JText::_('Clear'); ?></button>
		</div>
	...
	<table class="adminlist">
        ...
        count($videolist1['rs_showupload']); ?>);" />
			...
			<th class="left"><?php echo JHTML::_('grid.sort', 'Title', 'title', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']); ?>
			</th>
		...
		if ($n >= 1) {
			...
			<tr id="listItem_<?php echo $row_showupload->id; ?>" class="<?php echo 'row' . ($i % 2); ?>">
				...
				<td class="center">
					...
					$str1 = explode('administrator', JURI::base());
					$videopath1 = $str1[0];
					$videolink1 = 'index.php?option=com_contushdvideoshare&view=player&id=' . $row_showupload->id;
					...
					elseif ($row_showupload->filepath == "Url" || $row_showupload->filepath == "Youtube") {
						...
						if ($videolink2 != "") : ?>
						<a href="javascript:void(0)" <?php if($row_showupload->published == 1){ ?>
							onclick="window.open('<?php echo $videolink; ?>','','width=600,height=500,maximize=yes,menubar=no,status=no,location=yes,toolbar=yes,scrollbars=yes')" <?php } ?>>
