...
class Modelcontushdvideosharevideoupload extends JModel {
    ...
    function getupload() {
        ...
        $updatestreamerdata = $value = $updateform = $streamerpath = $streamname = $url = $previewurl = $flv = $hd = $hq = $ftype = $success = $editvideo1 = "";
        if (JRequest::getVar('type', '', 'get', 'string') == 'edit') {
            $editvideo = "SELECT  a.`id`, a.`memberid`, a.`published`, a.`title`, a.`seotitle`,
						 ...
						 FROM #__hdflv_upload a
             			 ...
             			 WHERE a.id=" . JRequest::getVar('id', '', 'get', 'int');
            $db = $this->getDBO();
...
//        $preview = "components/com_contushdvideoshare/images/default_preview.jpg";
        $seltype = JRequest::getVar('seltype', '', 'post', 'string');

        if (JRequest::getCmd('uploadbtn')) {
            if ($user->get('id')) {
                $memberid = $user->get('id'); //Setting the loginid into session
            }
            if ((JRequest::getVar('videourl', '', 'post', 'string') == "1" || JRequest::getVar('normalvideoformval', '', 'post', 'string')) && $seltype==1) { // Checking for normal file type of videos
                ...
                if ($uploadFile["name"] != "") {
                    ...
                    if ((($uploadFile["type"] == "image/gif") || ($uploadFile["type"] == "image/jpeg") || ($uploadFile["type"] == "image/pjpeg") || ($uploadFile["type"] == "image/png"))) {
...
//                        $previewurl = JURI::base() . "components/com_contushdvideoshare/images/default_preview.jpg";
            ...
            if($seltype==0){
                $ftype = "Youtube";
                $updatestreamerdata .= ",streamerpath='',streameroption=''";
            }
            else if($seltype==2){
                $ftype = "Url";
                $updatestreamerdata .= ",streamerpath='',streameroption=''";
               ...
               if (!empty($streamname)) {
                    ...
                    $updatestreamerdata .= ",streamerpath='$streamerpath',streameroption='$streameroption'";
                }
            }
...
//                $previewurl = JURI::base() .  "components/com_contushdvideoshare/images/default_preview.jpg";
            ...
            if (JRequest::getVar('videotype') == 'edit') {
                ...
                if($seltype==3 && $streameroption =="rtmp"){
                    $updateform .= ",hdurl=''";

                }
              $query = ' update #__hdflv_upload SET filepath="' . $ftype . '",tags= "' . $tags . '",title=' . $title . ',seotitle="' . $seoTitle . '",type="' . $type . '",download="' . $download . '",description=' . $description . $updateform .$updatestreamerdata. ' where id=' . JRequest::getVar('videoid', '', 'post', 'int');
