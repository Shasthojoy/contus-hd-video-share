 ...
 */
class contushdvideoshareModelcategory extends JModelList {
	...
	function getcategory() { 
		...
		$total = 0;		
		$filter_order = $mainframe->getUserStateFromRequest($option . 'filter_order_category', 'filter_order', 'a.category', 'cmd');
	...
	global $mainframe;
		...
		$db->setQuery($query);
		$category = $db->loadObject();
                if(isset($category->id) && $category->id!=0 && $category->id!=$id ){
