...
<!--			<select onchange="this.form.submit()" class="inputbox" name="member_upload">
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
			<?php
			$videoListId = JHTML::_('grid.sort', 'ID', 'member_id', $arrMemberFilter['filter_order_Dir'], $arrMemberFilter['filter_order']);
			$firstName = JHTML::_('grid.sort', 'Name', 'a.name', $arrMemberFilter['filter_order_Dir'], $arrMemberFilter['filter_order']);

			?>
				<th width="1%">#</th>
				<th width="2%"><input type="checkbox" name="toggle" value="" onclick="Joomla.checkAll(this)" />
				</th>
				<th width="20%" class="left">
                        <?php //echo JHTML::_('grid.sort', 'Name', 'a.name', @$arrMemberFilter['filter_order_Dir'], @$arrMemberFilter['filter_order']); ?>
                        <?php echo $firstName;  ?>
				</th>
				<th width="20%" class="left"><?php echo JHTML::_('grid.sort', 'User Name', 'a.username', @$arrMemberFilter['filter_order_Dir'], $arrMemberFilter['filter_order']);?>
				</th>
				<th width="20%" class="left">Email</th>
				<th width="20%">Joined Date</th>
				<th width="8%" class="center"><?php echo 'Allow Upload';?>
				</th>
...
//                        echo $states[$memberDetail->block][0];
			...
			if ($published == 0)
			{
				$statusImage = '<a title="Activate User" onclick="return listItemTask(\'cb' . $i . '\',\'unpublish\')" href="javascript:void(0);">
								<img src="components/com_contushdvideoshare/images/tick.png" />';
			}
			else
			{
				$statusImage = '<a title="Deactivate User" onclick="return listItemTask(\'cb' . $i . '\',\'publish\')" href="javascript:void(0);">
		...
		<tr class="<?php echo 'row' . ($i % 2); ?>">
			...
			if ($allowUpload == '1')
			{
				$allowUploadImage = '<a title="Allow Upload" onclick="return listItemTask(\'cb' . $i . '\',\'unallowupload\')" href="javascript:void(0);">
									 <img src="components/com_contushdvideoshare/images/tick.png" />';
			}
			else
			{
				$allowUploadImage = '<a title="Disallow Upload" onclick="return listItemTask(\'cb' . $i . '\',\'allowupload\')" href="javascript:void(0);">
									 <img src="components/com_contushdvideoshare/images/publish_x.png" />';
			}
			?> <?php echo $allowUploadImage; ?>
			</td>
			<td class="center"><?php echo $published1; ?></td>
	...
	<input type="hidden" name="filter_order" value="<?php echo 'member_id'; ?>" />
