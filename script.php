...
class Com_ContushdvideoshareInstallerScript
{
	...
	public function postflight($type, $parent)
	{
		...
		$player_colorscolumnExists = $player_valuescolumnExists = $imaadscolumnExists = $embedcodecolumnExists
						= $subtitle1codecolumnExists = $subtitle2codecolumnExists = $subtile_lang2codecolumnExists
										= $subtile_lang1codecolumnExists = $amazons3columnExists = $imaaddetcolumnExists
														= $dispenablecolumnExists = $sidethumbviewcolumnExists = 'false';
		...
		if (!empty($tablecolumnData))
		{
			...
			if ($player_iconscolumnExists == 'false')
			{
				...
				$query->clear()
						...
						->set($db->quoteName('player_icons') . ' = ' . $db->quote($arrplayer_icons));
				$db->setQuery($query);
				$db->query();
			}
			...
			if ($player_valuescolumnExists == 'false')
			{
				...
				$query->clear()
						->update($db->quoteName('#__hdflv_player_settings'))
						->set($db->quoteName('player_values') . ' = ' . $db->quote($arrplayer_values));
				$db->setQuery($query);
				$db->query();
			}
			...
			if ($thumbviewcolumnExists == 'false')
			{
				...
				$query->clear()
						...
						->set($db->quoteName('thumbview') . ' = ' . $db->quote($arrthumbview));
				$db->setQuery($query);
				$db->query();
			}
			...
			if ($homethumbviewcolumnExists == 'false')
			{
				...
				$query->clear()
						->update($db->quoteName('#__hdflv_site_settings'))
						->set($db->quoteName('homethumbview') . ' = ' . $db->quote($arrhomethumbview));
				$db->setQuery($query);
				$db->query();
			}
			...
			if ($sidethumbviewcolumnExists == 'false')
			{
				...
				$query->clear()
						->update($db->quoteName('#__hdflv_site_settings'))
						->set($db->quoteName('sidethumbview') . ' = ' . $db->quote($arrsidethumbview));
				$db->setQuery($query);
				$db->query();
			}
			else
			{
				...
				if (!empty($sidethumbviewResult))
				{
					if (!isset($upgradesidethumbview['siderandomvideorow']))
					{
						...
						$query->clear()
								->update($db->quoteName('#__hdflv_site_settings'))
								->set($db->quoteName('sidethumbview') . ' = ' . $db->quote($arrupgradesiderandomvideorow));
						$db->setQuery($query);
						$db->query();
					}
					...
					if (!isset($upgradesidethumbview['siderandomvideocol']))
					{
						...
						$query->clear()
								->update($db->quoteName('#__hdflv_site_settings'))
								->set($db->quoteName('sidethumbview') . ' = ' . $db->quote($arrupgradesiderandomvideocol));
						$db->setQuery($query);
						$db->query();
					}
				}
			...
			if ($dispenablecolumnExists == 'false')
			{
				...
				$query->clear()
						->update($db->quoteName('#__hdflv_site_settings'))
						->set($db->quoteName('dispenable') . ' = ' . $db->quote($arrdispenable));
				$db->setQuery($query);
				$db->query();
			}
			else
			{
				...
				if (!isset($upgradedisp['upload_methods']))
				{
					...
					$query->clear()
							->update($db->quoteName('#__hdflv_site_settings'))
							->set($db->quoteName('dispenable') . ' = ' . $db->quote($arrupgradedisp));
					$db->setQuery($query);
					$db->query();
				}
		...
		else
		{
			...
			$column_category = array(
				'id', 'member_id', 'category', 'seo_category', 'parent_id',
				'ordering', 'lft', 'rgt', 'published'
				);
			$query->clear()
					...
					->values(
							implode(
									',',
									array(
										1, 0, $db->quote('Speeches'), $db->quote('Speeches'), 0, 1, 19, 20, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										2, 0, $db->quote('Interviews'), $db->quote('Interviews'), 0, 2, 11, 12, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										3, 0, $db->quote('Talk Shows'), $db->quote('Talk-Shows-'), 0, 3, 21, 22, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										4, 0, $db->quote('News & Info'), $db->quote('News-Info'), 0, 4, 15, 16, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										5, 0, $db->quote('Documentary'), $db->quote('Documentary'), 0, 5, 7, 8, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										6, 0, $db->quote('Travel'), $db->quote('Travel'), 0, 6, 25, 26, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										7, 0, $db->quote('Cooking'), $db->quote('Cooking'), 0, 7, 5, 6, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										8, 0, $db->quote('Music'), $db->quote('Music'), 0, 8, 13, 14, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										9, 0, $db->quote('Trailers'), $db->quote('Trailers'), 0, 9, 23, 24, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										10, 0, $db->quote('Religious'), $db->quote('Religious'), 0, 10, 17, 18, 1
										)
									)
							)
					->values(
					...
					->values(
							implode(
									',',
									array(
										12, 0, $db->quote('Greetings'), $db->quote('Greetings'), 0, 12, 9, 10, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										13, 0, $db->quote('Comedy'), $db->quote('Comedy'), 0, 13, 3, 4, 1
										)
									)
							)
					->values(
							implode(
									',',
									array(
										14, 0, $db->quote('Actors'), $db->quote('Actors'), 0, 14, 1, 2, 1
										)
									)
							);
			...
			$query->clear()
					...
					->values(
							implode(
									',',
									array(
										1, 1, $db->quote($homethumbview), $db->quote($disenable), $db->quote($thumbview),
			...
			$db = JFactory::getDBO();
			$groupname = 8;
			$uploadcolumnsArray = array(
				'id', 'memberid', 'published', 'title', 'seotitle', 'featured', 'type', 'rate',
...
`catid` varchar(100) CHARACTER SET utf8 NOT NULL
		...
		foreach ($modules as $module)
		{
			...
			if ($result)
			{
				...
				JFile::move(
						$root . '/modules/' . $name . '/' . $name . '.j3.xml',
						$root . '/modules/' . $name . '/' . $name . '.xml'
						);
			}
		...
		JFile::move(
				$root . '/components/com_contushdvideoshare/views/category/tmpl/default.j3.xml',
				$root . '/components/com_contushdvideoshare/views/category/tmpl/default.xml'
				);
		...
		JFile::move(
				$root . '/plugins/content/hvsarticle/hvsarticle.j3.xml',
				$root . '/plugins/content/hvsarticle/hvsarticle.xml'
				);
		?>
		<style  type="text/css">
