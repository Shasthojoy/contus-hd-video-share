...
function com_install() {
    ...
    if ($ret) {
        ...
        $db =& $minst->getDBO();
        $query = "SELECT COUNT(id) as n FROM #__modules WHERE module = 'mod_HDVideoSharePopular'";
        $db->setQuery($query);
        $db->query();
        $n = $db->loadResult();
        if ($n > 1) {
            $query = "SELECT id FROM #__modules WHERE module = 'mod_HDVideoSharePopular'".
                " AND title = 'Popular Videos' and published = 0 ORDER BY id DESC LIMIT 1";
            $db->setQuery($query);
            $db->query();
            $m = $db->loadResult();
            if ($m) {
		...
		$query = "SELECT COUNT(id) as n FROM #__modules WHERE module = 'mod_HDVideoShareSearch'";
        $db->setQuery($query);
        $db->query();
        $n = $db->loadResult();
        if ($n > 1) {
            $query = "SELECT id FROM #__modules WHERE module = 'mod_HDVideoShareSearch'".
                " AND title = 'Search Videos' and published = 0 ORDER BY id DESC LIMIT 1";
            $db->setQuery($query);
            $db->query();
            $m = $db->loadResult();
            if ($m) {
		...
		$query = "SELECT COUNT(id) as n FROM #__modules WHERE module = 'mod_HDVideoShareRecent'";
        $db->setQuery($query);
        $db->query();
        $n = $db->loadResult();
        if ($n > 1) {
            $query = "SELECT id FROM #__modules WHERE module = 'mod_HDVideoShareRecent'".
                " AND title = 'Recent Videos' and published = 0 ORDER BY id DESC LIMIT 1";
            $db->setQuery($query);
            $db->query();
            $m = $db->loadResult();
            if ($m) {
		...
		$query = "SELECT COUNT(id) as n FROM #__modules WHERE module = 'mod_HDVideoShareFeatured'";
        $db->setQuery($query);
        $db->query();
        $n = $db->loadResult();
        if ($n > 1) {
            $query = "SELECT id FROM #__modules WHERE module = 'mod_HDVideoShareFeatured'".
                " AND title = 'Featured Videos' and published = 0 ORDER BY id DESC LIMIT 1";
            $db->setQuery($query);
            $db->query();
            $m = $db->loadResult();
            if ($m) {
		...
		$query = "SELECT COUNT(id) as n FROM #__modules WHERE module = 'mod_HDVideoShareRelated'";
        $db->setQuery($query);
        $db->query();
        $n = $db->loadResult();
        if ($n > 1) {
            $query = "SELECT id FROM #__modules WHERE module = 'mod_HDVideoShareRelated'".
                " AND title = 'Related Videos' and published = 0 ORDER BY id DESC LIMIT 1";
            $db->setQuery($query);
            $db->query();
            $m = $db->loadResult();
            if ($m) {
		...
		$query = "SELECT COUNT(id) as n FROM #__modules WHERE module = 'mod_HDVideoShareCategories'";
        $db->setQuery($query);
        $db->query();
        $n = $db->loadResult();
        if ($n > 1) {
            $query = "SELECT id FROM #__modules WHERE module = 'mod_HDVideoShareCategories'".
                " AND title = 'Video Category' and published = 0 ORDER BY id DESC LIMIT 1";
            $db->setQuery($query);
            $db->query();
            $m = $db->loadResult();
            if ($m) {
