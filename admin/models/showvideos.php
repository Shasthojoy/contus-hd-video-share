...
//            $filter_order ='ordering';
        ...
        if ($userId != 0) {
            $where = "where c.usertype='Super Administrator'";
        } else {
            $where = "where c.usertype!='Super Administrator'";
        }
        ...
        if ($filter_order) {
            ...
            $query = "SELECT distinct(d.videoid) as cvid,a.*,b.category FROM #__hdflv_upload a inner join #__users c on c.id = a.memberid LEFT JOIN #__hdflv_category b ON a.playlistid=b.id left join #__hdflv_comments d on d.videoid=a.id $where order by $filter_order $filter_order_Dir LIMIT $pageNav->limitstart,$pageNav->limit";
    ...
    function savevideos($task) {
        ...
        $memberid = (JRequest::getVar('user_id', '', 'post', 'int')) ? JRequest::getVar('user_id', '', 'post', 'int') : 62;
