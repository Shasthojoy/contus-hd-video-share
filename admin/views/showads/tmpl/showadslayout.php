...
if (version_compare(JVERSION, '3.0.0', 'ge'))
				{
	?>
				<input type="text" title="Search in module title." placeholder="Search Ads"
					   id="ads_search" name="ads_search" style="float:left; margin-right: 10px;">
				<div class="btn-group pull-left">
					<button type="submit"  class="btn hasTooltip">
						<i class="icon-search"></i>
					</button>
					<button class="btn hasTooltip" onclick="document.getElementById('ads_search').value = '';
...
<?php
				...
				<input type="text" title="Search in module title." value="
					<?php
					if (isset($arrAdsFilter['ads_search']))
					{
				...
				<button onclick="document.getElementById('ads_search').value = '';
...
<?php
		...
		<div class="filter-select fltrt" style="float: right;">
			<select onchange="this.form.submit()" class="inputbox" name="ads_status">
				...
				<option value="1" <?php
				if (isset($arrAdsFilter['ads_status']) && $arrAdsFilter['ads_status'] == '1')
				{
				...
				<option value="2"
					<?php
					if (isset($arrAdsFilter['ads_status']) && $arrAdsFilter['ads_status'] == '2')
					{
						echo 'selected=selected';
					}
					?>>Unpublished</option>
				<option value="3" 
					<?php
					if (isset($arrAdsFilter['ads_status']) && $arrAdsFilter['ads_status'] == '3')
					{
						echo 'selected=selected';
					}
					?>>Trashed</option>
			...
			<select onchange="this.form.submit()" class="inputbox" name="ads_type">
				...
				<option value="1" 
					<?php
					if (isset($arrAdsFilter['ads_type']) && $arrAdsFilter['ads_type'] == '1')
					{
						echo 'selected=selected';
					}
				...
				<option value="2" 
					<?php
					if (isset($arrAdsFilter['ads_type']) && $arrAdsFilter['ads_type'] == '2')
					{
						echo 'selected=selected';
					}
					?>>Mid Roll</option>
				<option value="3" 
					<?php
					if (isset($arrAdsFilter['ads_type']) && $arrAdsFilter['ads_type'] == '3')
					{
						echo 'selected=selected';
					}
					?>>IMA</option>
	...
	<table  class="adminlist 
		...
		<thead>
			<tr>
				...
				{
?> Joomla.checkAll(this)<?php
				}
				?>" />
				</th>
				<th class="left">
<?php
echo JHTML::_('grid.sort', 'Ad Name', 'adsname', @$arrAdsFilter['filter_order_Dir_ads'], @$arrAdsFilter['filter_order_ads']);
?>
				</th>	
				<th>
<?php
...
?>
				...
				<th 
					...
					?>>
<?php
echo JHTML::_('grid.sort', 'Published', 'published', @$arrAdsFilter['filter_order_Dir_ads'], @$arrAdsFilter['filter_order_ads']);
?>
				</th>
				<th
					<?php
					...
					?>>
<?php
echo JHTML::_('grid.sort', 'Ad Visits', 'clickcounts', @$arrAdsFilter['filter_order_Dir_ads'], @$arrAdsFilter['filter_order_ads']);
?>
				</th>
				<th 
					<?php
