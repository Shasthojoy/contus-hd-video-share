	...
	?>
			...
			if (bol_file4 == true)
					...
					if (document.getElementById('videourl').value == "")
							...
							var theurl = document.getElementById("videourl").value;
								if (
										theurl.contains("youtube.com") ||
										theurl.contains("vimeo.com") ||
										theurl.contains("youtu.be") ||
										theurl.contains("dailymotion") ||
										theurl.contains("viddler")
							)
								{
...
<div style="position: relative;">
	<fieldset class="adminform">
		...
		?>
		<table id="video_upload_part" class="admintable <?php
		if (version_compare(JVERSION, '3.0.0', 'ge'))
		{
		...
		?>">
			...
			?>
			<tr>
				<td><?php echo JHTML::tooltip('Select streamer option', 'Streamer Option', '', 'Streamer Option');
			?></td>
				<td <?php
				if (version_compare(JVERSION, '3.0.0', 'ge'))
				{
			...
			<tr id="stream1" name="stream1"><td><?php echo JHTML::tooltip('Select Streamer Path', 'Streamer Path', '', 'Streamer Path');
			?></td>
				<td>
					<input type="text" name="streamname"  id="streamname" style="width:300px"
						   maxlength="250" value="<?php echo $editVideo['rs_editupload']->streamerpath; ?>" />
				</td>
			</tr>
			...
			?></td>
				...
				?>>
					...
					if (isset($player_values['licensekey']) && $player_values['licensekey'] != '')
						{
						?>
						<input type="radio" name="filepath" id="filepath5" <?php echo $embedchk; ?> value="Embed" 
							   onclick="fileedit('Embed');" /><span>Embed Video</span>
					<?php
					}
						?>
				</td></tr>
			...
			<tr id="ffmpeg_disable_new2" name="ffmpeg_disable_new2">
				<td><?php echo JHTML::tooltip('Select Hdvideo to upload', 'Upload HD Video', '', 'Upload HD Video(optional)');?></td>
				<td>
					<div id="f2-upload-form" >
						<form name="hdvideoform" method="post" enctype="multipart/form-data" >
							...
							?>
								   value="Upload HD Video" style="margin: 2px 0 0 5px;" disabled="disabled" onclick="addQueue(this.form.name);" />
							<label><?php
							if ($editVideo['rs_editupload']->filepath == 'File')
							{
				...
				<td>
					<div id="f3-upload-form" >
						<form name="thumbimageform" method="post" enctype="multipart/form-data" >
							...
							?>
								   value="Upload Thumb Image" style="margin: 2px 0 0 5px;" disabled="disabled" onclick="addQueue(this.form.name);" />
							<label>
								<?php
								if ($editVideo['rs_editupload']->filepath == 'File' || $editVideo['rs_editupload']->filepath == 'Embed')
								{
			...
			<tr id="ffmpeg_disable_new4" name="ffmpeg_disable_new4">
				<td><?php echo JHTML::tooltip('Select preview image to upload', 'Upload Preview Image', '', 'Upload Preview Image(optional)');?>
				</td>
				<td>
					<div id="f4-upload-form" >
						<form name="previewimageform" method="post" enctype="multipart/form-data" >
							...
							?> value="Upload Preview Image" style="margin: 2px 0 0 5px;" disabled="disabled" onclick="addQueue(this.form.name);" />
							<label><?php
							if ($editVideo['rs_editupload']->filepath == 'File')
							{
...
?>"/>
			...
			<tr id="fvideos" name="fvideos"> <td><?php echo JHTML::tooltip('Select video to upload', 'Upload Video', '', 'Upload Video');
					?></td>
				<td>
			...
			<tr id="subtitle_video_srt1" name="subtitle_video_srt1"><td>
				<?php echo JHTML::tooltip('Select srt file to upload', 'Upload Video Subtitle1', '', 'Upload Video Subtitle1');
					?></td><td>
					<div id="f7-upload-form" >
						<form name="subtitle_video_srt1form" method="post" enctype="multipart/form-data" >
							...
							<input type="button" name="uploadBtn" <?php
								...
								disabled="disabled" onclick="addQueue(this.form.name);" />
							<label><?php
			...
			<tr id="subtilelang1" style="display:none;"><td width="17%">
				<?php echo JHTML::tooltip('Enter subtile1 language', 'Subtile language', '', 'Subtile1 language'); ?></td>
				<td width="83%">
					<input type="text" name="subtitle_lang1"  id="subtitle_lang1" style="width:300px" maxlength="250"
						   value="<?php echo htmlentities($editVideo['rs_editupload']->subtile_lang1); ?>" /></td></tr>
			<tr id="subtitle_video_srt2" name="subtitle_video_srt2"><td>
				<?php echo JHTML::tooltip('Select srt file to upload', 'Upload Video Subtitle2', '', 'Upload Video Subtitle2');
					?></td><td>
					<div id="f8-upload-form" >
						<form name="subtitle_video_srt2form" method="post" enctype="multipart/form-data" >
							...
							<input type="button" name="uploadBtn" <?php
								...
								?> value="Upload Video Subtitle2"
								style="margin: 2px 0 0 5px;" disabled="disabled" onclick="addQueue(this.form.name);" />
							<label><?php
							if ($editVideo['rs_editupload']->filepath != 'Embed')
							{
			...
			<tr id="subtilelang2" style="display:none;"><td width="17%">
				<?php echo JHTML::tooltip('Enter subtile2 language', 'Subtile2 language', '', 'Subtile2 language'); ?></td>
				<td width="83%"><input type="text" name="subtitle_lang2"  id="subtitle_lang2" style="width:300px"
									   maxlength="250" value="<?php echo htmlentities($editVideo['rs_editupload']->subtile_lang2); ?>" />
				</td></tr>
		</table>
	</fieldset>
</div>
...
<form action='index.php?option=com_contushdvideoshare&layout=adminvideos<?php 
echo (JRequest::getVar('user', '', 'get') == 'admin') ? "&user=" . JRequest::getVar('user', '', 'get') : ''; ?>'
method="post" name="adminForm" id="adminForm" enctype="multipart/form-data" style="float: none;position: relative;">
	<fieldset class="adminform">
		<?php
		if (!version_compare(JVERSION, '3.0.0', 'ge'))
				{
		...
		?>  width="100%">
			...
			<tr><td width="17%"><?php echo JHTML::tooltip('Enter title for the video', 'Title', '', 'Title');
		?></td><td width="83%"><input type="text" name="title"  id="title" style="width:300px" maxlength="250"
											value="<?php echo htmlentities($editVideo['rs_editupload']->title); ?>" /></td>
			</tr>
			<tr><td><?php echo JHTML::tooltip('Enter description for the video', 'Description', '', 'Description');
		?></td><td>
					<?php
					$imageDesc = "";
			...
			<tr><td><?php echo JHTML::tooltip('Enter tags for the video', 'Tags', '', 'Tags');
					?></td><td><input type="text" name="tags"  id="tags" style="width:300px;float:left;"
								maxlength="250" value="<?php echo $editVideo['rs_editupload']->tags; ?>" />
					<label>Separate tags by space</label></td></tr>
			<tr id="target"><td><?php echo JHTML::tooltip('Enter target url for the video (Not supported for vimeo)', 'Target URL', '', 'Target URL');
					?></td>
				<td><input type="text" name="targeturl" id="targeturl" style="width:300px" maxlength="250"
...
?>
			...
			<tr><td><?php echo JHTML::tooltip('Select option to filter categories', 'Filter by Category', '', 'Filter by Category');
?></td>
				<td <?php
				if (version_compare(JVERSION, '3.0.0', 'ge'))
				{
				...
				?>>
					<input type="radio" name="playliststart" id='playliststart0' value="0z"  <?php echo 'checked'; ?>
						   onchange="select_alphabet('0z')" />All&nbsp;&nbsp;
			...
			<tr><td><?php echo JHTML::tooltip('Select category for the video', 'Category', '', 'Category');
?></td><td>
			...
			<tr>
				<td><?php echo JHTML::tooltip('Enter order for the video', 'Order', '', 'Order');
						?></td>
				<td><input type="text" name="ordering"  id="ordering" style="width:50px" maxlength="250" value="<?php
				echo $editVideo['rs_editupload']->ordering;
				?>"/></td>
			</tr>
			<tr>
				<td><?php echo JHTML::tooltip('Select user access level (Not supported for vimeo)', 'User Acess Level', '', 'User Acess Level');
						?></td>
				<td>
...
?>
			...
			<tr id="postroll-ad"><td>
				<?php echo JHTML::tooltip('Post-roll ads (Not supported for vimeo)', 'Post-roll Ad', '', 'Post-roll Ad');
?></td>
						<?php
						$postRollEnable = $postRollDisable = '';
			...
			<tr id="postroll"><td class="key"><?php echo JHTML::tooltip('Post-roll Name (Not supported for vimeo)', 'Post-roll Name', '', 'Post-roll Name');
						?></td><td>
...
if ($count >= 1)
{
	for ($i = 0; $i < $count; $i++)
	{
		...
		?>
						...
						<?php
						$prerolladsEnable = $prerolladsDisable = '';
			...
			<tr id="preroll-ad"><td><?php echo JHTML::tooltip('Pre-roll ads (Not supported for vimeo)', 'Pre-roll Ad', '', 'Pre-roll Ad');
						?></td>
				<td <?php
				if (version_compare(JVERSION, '3.0.0', 'ge'))
				{
			...
			<tr id="preroll"><td class="key"><?php echo JHTML::tooltip('Pre-roll Name (Not supported for vimeo)', 'Pre-roll Name', '', 'Pre-roll Name');
						?></td>
				<td>
			...
			<tr id="download"><td>
				<?php echo JHTML::tooltip('Download Video (Not supported for vimeo, youtube and streamer)', 'Download Video', '', 'Download Video');
					?></td>
				<td <?php
				if (version_compare(JVERSION, '3.0.0', 'ge'))
				{
			...
			<tr><td><?php echo JHTML::tooltip('Option to enable/disable video', 'Status', '', 'Status');
					?></td>
				<td <?php
				if (version_compare(JVERSION, '3.0.0', 'ge'))
				{
				...
				?>>
					<select name="published" id="published">
						<option value="1" <?php
						if (isset($editVideo['rs_editupload']->published) && $editVideo['rs_editupload']->published == 1)
						{
							echo 'selected';
						}
							?>>Published</option>
						<option value="0" <?php
						if (isset($editVideo['rs_editupload']->published) && $editVideo['rs_editupload']->published == 0)
						{
							echo 'selected';
						}
							?>>Unpublished</option>
...
elseif ($editVideo['rs_editupload']->filepath == 'Url')
{
	?>
				fileedit('Url');
...
elseif ($editVideo['rs_editupload']->filepath == 'Youtube')
{
	?>
				fileedit('Youtube');
...
elseif ($editVideo['rs_editupload']->filepath == 'FFmpeg')
{
	?>
				fileedit('FFmpeg');
...
elseif ($editVideo['rs_editupload']->filepath == 'Embed')
{
	?>
				fileedit('Embed');
