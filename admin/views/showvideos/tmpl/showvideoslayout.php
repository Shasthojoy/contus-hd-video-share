...
// Used for class="hasTip3" titles
$filename = 'testtooltip.js';
	...
	dragdr(document).ready(function() {
		dragdr("#test-list").sortable({
			...
			update: function() {
				...
				dragdr.post("<?php
				echo $baseurl;
				?>/index.php?option=com_contushdvideoshare&task=videos&layout=sortorder&pagenum=<?php
				echo $current_page;
				?>", order);
...
else
{
	...
	<form action="index.php?option=com_contushdvideoshare&layout=adminvideos<?php echo $userUrl; ?>"
		...
		<fieldset id="filter-bar" 
			...
			<div class="filter-search fltlft" style="float: left;">
				...
				if (!version_compare(JVERSION, '3.0.0', 'ge'))
		{
					?><label for="search" class="filter-search-lbl">Filter:</label>
				<input type="text" title="Search in module title." value="
					<?php
					if (isset($videolist1['lists']['search']))
					{
			...
			?>" id="search" name="search">
				...
				this.form.submit();" type="button" style="padding: 1px 6px;">
					...
					{
						?>
					<input type="text" title="Search in module title." placeholder="Search Videos"
						   id="search" name="search" style="float: left; margin-right: 10px; ">
					<div class="btn-group pull-left">
						<button type="submit" class="btn hasTooltip">
							<i class="icon-search"></i>
						</button>
						<button onclick="document.getElementById('search').value = '';
				this.form.submit();" type="button" class="btn hasTooltip">
	...
	<?php
			...
			?>" style="float: right; margin-left: 5px;">
				<select onchange="this.form.submit()" class="
					<?php
					if (!version_compare(JVERSION, '3.0.0', 'ge'))
					{
						?>inputbox
			...
			?>" name="filter_state">
					...
					<option value="1" 
						<?php
						if (isset($videolist1['lists']['state_filter']) && $videolist1['lists']['state_filter'] == '1')
						{
					...
					<option value="2" 
						<?php
						if (isset($videolist1['lists']['state_filter']) && $videolist1['lists']['state_filter'] == '2')
						{
							echo 'selected=selected';
						}
						?>>Unpublished</option>
					<option value="3" 
						<?php
						if (isset($videolist1['lists']['state_filter']) && $videolist1['lists']['state_filter'] == '3')
						{
							echo 'selected=selected';
						}
						?>>Trashed</option>
			...
			?></div>
				...
				<?php
					...
					<option value="1" 
						<?php
						if (isset($videolist1['lists']['featured_filter']) && $videolist1['lists']['featured_filter'] == '1')
						{
							echo 'selected=selected';
						}
						?>>Featured</option>
					<option value="2" 
						<?php
						if (isset($videolist1['lists']['featured_filter']) && $videolist1['lists']['featured_filter'] == '2')
						{
							echo 'selected=selected';
						}
						?>>Unfeatured</option>
	...
	foreach ($videolist1['rs_showplaylistname'] as $arrCategories)
		...
		?>		
						<option value="<?php
						echo $arrCategories->id;
						?>" 
							<?php
							if (isset($videolist1['lists']['category_filter']) && $videolist1['lists']['category_filter'] == $arrCategories->id)
							{
								echo 'selected=selected';
							}
							?>>
								<?php
								echo $arrCategories->category;
								?>
						</option>					
	<?php
	}
	...
	<?php
			...
			<th class="left">
<?php
echo JHTML::_('grid.sort', 'Title', 'a.title', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			...
			<th>
<?php
echo JHTML::_('grid.sort', 'Category', 'playlistid', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
			<th width="5%">
<?php
echo JHTML::_('grid.sort', 'Viewed', 'times_viewed', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
	...
	<?php
		...
		?>
			<th>
<?php
echo JHTML::_('grid.sort', 'Video Link', 'videourl', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
			<th>
<?php
echo JHTML::_('grid.sort', 'Thumb Link', 'thumburl', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
			<th>Postroll Ad</th>
			<th>Preroll Ad</th>
			...
			<th>
<?php
echo JHTML::_('grid.sort', 'Order', 'ordering', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
			<th width="4%">
<?php
echo JHTML::_('grid.sort', 'Status', 'published', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
			<th width="4%">
<?php
echo JHTML::_('grid.sort', 'Featured', 'featured', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
			<th width="2%">
<?php
echo JHTML::_('grid.sort', 'ID', 'Id', @$videolist1['lists']['order_Dir'], @$videolist1['lists']['order']);
?>
			</th>
			</thead>
						...
						<tr id="listItem_<?php
							...
							<td class="left">
								<a href="<?php echo $link; ?>"
								   style="word-wrap: break-word;width: 130px;display: block;">
<?php
echo $row_showupload->title;
?>
								</a>
							</td>
							<td align="center"><?php
								if (isset($row_showupload->cvid))
								{
...
$str1 = explode('administrator', JURI::base());
$videopath1 = $str1[0];
$videolink1 = 'index.php?option=com_contushdvideoshare&view=player&id='
		. $row_showupload->id . '&catid=' . $row_showupload->playlistid . '&adminview=true';
...
?>
										...
										if ($row_showupload->published == 1)
											{
											?>
onclick="window.open('<?php echo $videolink; ?>', '',
	'width=300,height=200,maximize=yes,menubar=no,status=no,location=yes,toolbar=yes,scrollbars=yes')" 
												   <?php
										}
											?>>
...
onclick="window.open('<?php echo $videolink; ?>', '',
			'width=600,height=500,maximize=yes,menubar=no,status=no,location=yes,toolbar=yes,scrollbars=yes')" >
...
?>
								...
								elseif ($row_showupload->filepath == "Url" || $row_showupload->filepath == "Youtube")
								{
									...
									if ($thumblink2 != "")
									{
										?>
										<a href="javascript:void(0)"
										   onClick="window.open('<?php echo trim($thumblink2); ?>', '',
													'width=600,height=500,menubar=yes,status=yes,location=yes,toolbar=yes,scrollbars=yes')">
