...
<?php } ?>
                    ...
                    <?php if(JRequest::getVar('id','','get','int') && JRequest::getvar('catid','','get','int'))
                    {
           ...
           $sum=count($this->commenttitle1);
            if($sum!=0){?>
                    ...
                    <?php  $j=0; foreach($this->commenttitle1 as $row){ ?>
...
{ ?>
                      ...
                      <div class="reply changecomment1"><a class="cursor_pointer"onclick="textdisplay(<?php echo $row->id;?>); parentvalue(<?php if($row->parentid !=0){echo $row->parentid;}else {echo $row->id;}?>)" title="Reply for this comment" value="1" id="hh">Reply</a></div>
...
<?php $page='index.php?option=com_contushdvideoshare&view=player&id='.JRequest::getVar('id','','get','int'); ?>
