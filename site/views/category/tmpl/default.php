	...
	if (isset($dispenable['categoryplayer']) && $dispenable['categoryplayer'] == 1)
	{
		 ...
		 */
				...
				if (($this->categoryview[0]->filepath == 'Embed')
					|| (!empty($this->categoryview[0])
					&& (preg_match('/vimeo/', $this->categoryview[0]->videourl))
					&& ($this->categoryview[0]->videourl != '')))
				{
					if ($this->homepageaccess == 'true')
					{
						...
						{
							// For vimeo videos
							$split = explode("/", $this->categoryview[0]->videourl);
							contushdvideoshareController::videohitCount_function($this->categoryview[0]->id);
							...
							<div id="flashplayer">
								...
								echo 'http:// player.vimeo.com/video/' . $split[3]
										. '?title=0&amp;byline=0&amp;portrait=0'; ?>" class="iframe_frameborder">
								</iframe>
							</div>
						<?php
						}
				...
				?>                                     
						...
						<div id="htmlplayer">
							...
							if ($this->homepageaccess == 'true')
							{
								...
								if ($this->categoryview[0]->filepath == "File"
									|| $this->categoryview[0]->filepath == "FFmpeg"
									|| $this->categoryview[0]->filepath == "Url")
								{
									$current_path = "components/com_contushdvideoshare/videos/";

									if ($this->categoryview[0]->filepath == "Url")
									{
										// For URL Method videos
										if ($this->categoryview[0]->streameroption == 'rtmp')
										{
											$rtmp = str_replace(
													'rtmp', 'http', $this->categoryview[0]->streamerpath
													);
											$video = $rtmp . '_definst_/mp4:' . $this->categoryview[0]->videourl
													. '/playlist.m3u8';
								...
								elseif ($this->categoryview[0]->filepath == "Youtube")
								{
									// For youtube videos
									...
									elseif (strpos($this->categoryview[0]->videourl, 'dailymotion') > 0)
									{
									...
									elseif (strpos($this->categoryview[0]->videourl, 'viddler') > 0)
									{
										// For viddler videos
							...
							<li class="video-item">
								<div class="home-thumb">
									...
									<div class="show-title-container">
										...
										echo $categoryCategoryVal;
										?>&<?php
										echo $categoryVideoVal;
										?>"
