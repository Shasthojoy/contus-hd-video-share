...
class Modelcontushdvideosharevideoupload extends ContushdvideoshareModel
{
	...
	public function getupload()
	{
		...
		$value = $updateform = $streamerpath = $streameroption = $streamname = $url
						= $previewurl = $flv = $hd = $hq = $ftype = $success = $editvideo1 = '';
		$s3status = 0;
		$flagVideo = 1;
		...
		if ($task_edit == 'edit')
		{
			...
			if ($flagVideo != 1)
			{
				$query->clear()
						->select(
								array(
									'a.id', 'a.memberid', 'a.published', 'a.title', 'a.seotitle', 'a.islive',
		...
		if ($uploadbutton)
		{
			...
			else
			{
				...
				$columns = array(
					...
					'type', 'memberid', 'description', 'created_date', 'usergroupid',
				...
				$values = array(
					...
					$db->quote($download), $db->quote($useraccess)
				);
				$query->clear()
	 ...
	 * 
	 * @return  ezffmpeg_common_time_to_seconds
	 */
