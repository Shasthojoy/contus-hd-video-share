...
class Modelcontushdvideoshareplayer extends JModel {
    ...
    function getVideoId($video) {
        $db = & JFactory::getDBO();
        $query = 'select id,playlistid from #__hdflv_upload where seotitle="' . $video . '"';
        $db->setQuery($query);
...
//              session_start();
    ...
    function getHTMLVideoDetails($videoId) {
        ...
        $db = $this->getDBO();
        $query = "select filepath,videourl,videos from #__hdflv_upload where " . $condition; //Query is to select the popular videos row
        $db->setQuery($query);
        $rows = $db->LoadObject();
        return $rows;
    }
