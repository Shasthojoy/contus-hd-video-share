...
class Modelcontushdvideoshareplayer extends JModel {
	...
	function ratting($videoid, $categoryid) {
		...
		if (JRequest::getVar('rate', '', 'get', 'int')) {
			$query = "UPDATE #__hdflv_upload
            			   SET rate=" . JRequest::getVar('rate', '', 'get', 'int') . "+rate,ratecount=1+ratecount 
            			   WHERE id=$id";
			$db->setQuery($query);
			$db->query();
	...
	function getHTMLVideoAccessLevel(){
		...
		if (JRequest::getvar('id', '', 'get', 'int')) {
            ...
            if(JRequest::getVar('video', '', 'default', 'int')){
                $videoIdInt=JRequest::getVar('video', '', 'default', 'int');
            }elseif(JRequest::getVar('video', '', 'default', 'string')){
			...
			if ($videoName != "") {
				...
				$query = "SELECT distinct a.*,b.category
                		  FROM #__hdflv_upload a 
                		  LEFT JOIN #__hdflv_category b on a.playlistid=b.id OR a.playlistid=b.parent_id 
                		  WHERE a.published='1' AND a.seotitle='$videoName'";
				$db->setQuery($query);
				$rowsVal = $db->loadAssoc();
            }elseif($videoIdInt !=0){
                $query = "select distinct a.*,b.category from #__hdflv_upload a left join #__hdflv_category b on a.playlistid=b.id or a.playlistid=b.parent_id where a.published='1' and a.id=$videoIdInt ";
                $db->setQuery($query);
                $rowsVal = $db->loadAssoc();
			}
		} else {
